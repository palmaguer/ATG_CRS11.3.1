@echo off
rem
rem Copyright (C) 1994, 2018, Oracle and/or its affiliates.
...
set CLASSPATH=%CLASSPATH%;%ENDECA_ROOT%\lib\java\mail-1.4.jar
...
set CLASSPATH=%CLASSPATH%;%ENDECA_ROOT%\lib\java\commons-lang3-3.7.jar

rem Add CAS jars to the classpath
if defined CAS_ROOT (
  for %%i in ("%CAS_ROOT%\lib\cas-dt\*.jar") do call set CLASSPATH=%%CLASSPATH%%;%%i%%
) else (
