 ...
 * 
 * @author ATG
 * @version $Id: //hosting-blueprint/B2CBlueprint/version/11.3.1/EStore/src/atg/projects/store/email/TesterEmailTools.java#3 $$Change: 1536476 $
...
public class TesterEmailTools extends StoreEmailTools {
  ...
  public static final String CLASS_VERSION =
    "$Id: //hosting-blueprint/B2CBlueprint/version/11.3.1/EStore/src/atg/projects/store/email/TesterEmailTools.java#3 $$Change: 1536476 $";
  ...
  public boolean isValidProductId(String pProductId, String pSiteId) throws RepositoryException{
    ...
    if (it != null) {      
      String siteIds = getSiteIdsValue(it);
   ...
   */
  protected String getSiteIdsValue(RepositoryItem pItem) throws RepositoryException{
    Object sites = pItem.getPropertyValue(SITE_IDS_PARAMETER);
    if (!pItem.getItemDescriptor().isContextMembershipEnabled()) { return sites.toString();}
    try{
      Collection<String> siteMemberships = pItem.getContextMemberships();
      if(!siteMemberships.isEmpty()){
        RepositoryPropertyDescriptor membershipProperty = pItem.getItemDescriptor().getContextMembershipProperty();
        if (membershipProperty != null && membershipProperty.getName().equals(SITE_IDS_PARAMETER))
          sites = SiteMembershipManager.getSiteMembershipManager().getSiteIds(siteMemberships);
      }
    } catch (SiteContextException e){
      if(isLoggingError()){
        logError("Could not get SiteMemberships " 
